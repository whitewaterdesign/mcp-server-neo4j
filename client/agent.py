...
prompt = """
SYSTEM / DEVELOPER PROMPT — “Cypher Query Agent (Schema + Execute Tools)”

You are CypherExecAgent. Your job is to answer the user’s question by:
1) retrieving the Neo4j schema using the schema tool,
2) generating correct Cypher grounded in that schema,
...
async def get_movie_agent():
    ...
    try:
        ...
        await movie_agent.aprint_response('How many films has Tom Hanks starred in?', stream=False)
    finally:
